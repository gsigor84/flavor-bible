...
export default function NandosMayoMarinadeStepper() {
  ...
  return (
    <div className="p-4 max-w-md mx-auto bg-white space-y-6 rounded-xl">
      {/* Stepper Nav */}
      ...
      {step === 1 && (
        <div className="space-y-4">
          ...
          {Object.entries(marinade).map(([ingredient, grams]) => (
            <div key={ingredient} className="flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => decrement(ingredient)} className="px-2 py-1 text-lg font-bold border rounded hover:bg-gray-100">
                  -
                </button>
                <input
                  type="number"
                ...
                />
                <button onClick={() => increment(ingredient)} className="px-2 py-1 text-lg font-bold border rounded hover:bg-gray-100">
                  +
                </button>
              </div>
            </div>
          ...
          <div className="flex justify-between pt-4 border-t text-sm font-medium text-gray-700">
            ...
            <button onClick={() => setStep(2)} className="px-4 py-2 bg-black text-white rounded hover:bg-gray-800">
              Next
            </button>
          </div>
        </div>
      ...
      {step === 2 && (
        <div className="space-y-4 text-sm text-gray-800">
          ...
          <p><strong>How to Make:</strong> Whisk all ingredients until emulsified.</p>
          <p><strong>How to Use:</strong> Ideal for chicken thighs or drumsticks. Grill or bake until golden.</p>
          <p><strong>Marination Time:</strong> 2–6 hours.</p>

          <div className="flex justify-between pt-4 border-t">
            <button onClick={() => setStep(1)} className="px-4 py-2 bg-gray-100 rounded hover:bg-gray-200 text-sm">
              Back
            </button>
            <button onClick={resetMarinade} className="px-4 py-2 bg-gray-800 text-white rounded hover:bg-gray-700 text-sm">
              Reset
            </button>
          </div>
        </div>
